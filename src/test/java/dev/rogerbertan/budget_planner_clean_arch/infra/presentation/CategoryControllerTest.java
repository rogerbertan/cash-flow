...
import dev.rogerbertan.budget_planner_clean_arch.infra.dto.CategoryUpdateRequest;
import dev.rogerbertan.budget_planner_clean_arch.infra.mapper.CategoryCreateMapper;
import dev.rogerbertan.budget_planner_clean_arch.infra.mapper.CategoryResponseMapper;
import dev.rogerbertan.budget_planner_clean_arch.infra.mapper.CategoryUpdateRequestMapper;
...
class CategoryControllerTest {
    ...
    @Mock
    private CategoryUpdateRequestMapper categoryUpdateRequestMapper;
    ...
    void getAllCategories_ShouldReturnEmptyList_WhenNoCategoriesExist() {
        ...
        when(categoryResponseMapper.toListDTO(Collections.emptyList())).thenReturn(Collections.emptyList());
    ...
    void updateCategory_ShouldReturnUpdatedCategory_WhenCategoryExists() {
        // Arrange
        Long categoryId = 1L;
        CategoryUpdateRequest request = TestDataFactory.createCategoryUpdateRequest("Updated Name", Type.INCOME);
        ...
        CategoryResponse response = TestDataFactory.createCategoryResponse(1L, "Updated Name", Type.INCOME);

        when(findCategoryByIdUseCase.execute(categoryId)).thenReturn(existingCategory);
        when(categoryUpdateRequestMapper.merge(existingCategory, request)).thenReturn(mergedCategory);
    ...
    void updateCategory_ShouldReturn404_WhenCategoryDoesNotExist() {
        ...
        CategoryUpdateRequest request = TestDataFactory.createCategoryUpdateRequest("Updated", Type.INCOME);
    ...
    void updateCategory_ShouldMergeExistingAndRequest_WhenUpdating() {
        ...
        CategoryResponse response = TestDataFactory.createCategoryResponse(1L, "Updated", Type.INCOME);
        ...
        verify(categoryUpdateRequestMapper, times(1)).merge(existingCaptor.capture(), requestCaptor.capture());

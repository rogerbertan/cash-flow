...
class GetMonthlySummaryUseCaseTest {
    ...
    @InjectMocks private GetMonthlySummaryUseCase useCase;
    ...
    void execute_ShouldPassCorrectParameters_WhenCalled() {
        ...
        verify(transactionGateway, times(1))
                .getMonthlyIncome(monthCaptor.capture(), yearCaptor.capture());
        verify(transactionGateway, times(1))
                .getMonthlyExpense(monthCaptor.capture(), yearCaptor.capture());
    ...
    void execute_ShouldRoundAllValues_WhenDecimalsPresent() {
        ...
        assertThat(result.totalIncome())
                .isEqualByComparingTo(new BigDecimal("1500.56").setScale(2, RoundingMode.HALF_UP));
        assertThat(result.totalExpense())
                .isEqualByComparingTo(new BigDecimal("800.44").setScale(2, RoundingMode.HALF_UP));
        assertThat(result.netBalance())
                .isEqualByComparingTo(new BigDecimal("700.12").setScale(2, RoundingMode.HALF_UP));
        assertThat(result.totalIncome().scale()).isEqualTo(2);
        assertThat(result.totalExpense().scale()).isEqualTo(2);
        assertThat(result.netBalance().scale()).isEqualTo(2);
    }

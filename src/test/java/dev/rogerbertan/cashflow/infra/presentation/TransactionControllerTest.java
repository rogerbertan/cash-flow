...
import dev.rogerbertan.cashflow.application.usecases.transaction.*;
...
class TransactionControllerTest {
    ...
    @Mock private FindAllTransactionUseCase findAllTransactionUseCase;

    @Mock private FindTransactionByIdUseCase findTransactionByIdUseCase;
    ...
    @Mock private SuggestTransactionCategoryUseCase suggestTransactionCategoryUseCase;

    @Mock private TransactionResponseMapper transactionResponseMapper;

    @Mock private TransactionCreateMapper transactionCreateMapper;

    @Mock private TransactionUpdateRequestMapper transactionUpdateRequestMapper;

    @Mock private CategorySuggestionMapper categorySuggestionMapper;
    ...
    void getAllTransactions_ShouldReturnPagedResults_WhenTransactionsExist() {
        ...
        Page<Transaction> transactionPage =
                new PageImpl<>(List.of(transaction1, transaction2), pageable, 2);
        ...
        // Act
        ResponseEntity<Page<TransactionResponse>> result =
    ...
    void createTransaction_ShouldReturnCreatedTransaction_WhenValidRequestProvided() {
        ...
        Transaction inputTransaction =
                TestDataFactory.createTransaction(
                        null, TestDataFactory.createIncomeCategory(), new BigDecimal("1000.00"));
    ...
    void updateTransaction_ShouldReturnUpdatedTransaction_WhenTransactionExists() {
        ...
        TransactionUpdateRequest request =
                TestDataFactory.createTransactionUpdateRequest(
                        Type.INCOME, new BigDecimal("1500.00"), "Updated description", category);
        Transaction existingTransaction = TestDataFactory.createIncomeTransaction();
        Transaction mergedTransaction =
                TestDataFactory.createTransaction(1L, category, new BigDecimal("1500.00"));
        Transaction updatedTransaction =
                TestDataFactory.createTransaction(1L, category, new BigDecimal("1500.00"));
        TransactionResponse response =
                TestDataFactory.createTransactionResponse(
                        1L,
                        Type.INCOME,
        ...
        when(transactionUpdateRequestMapper.merge(existingTransaction, request))
                .thenReturn(mergedTransaction);
    ...
    void updateTransaction_ShouldMergeExistingAndRequest_WhenUpdating() {
        ...
        TransactionUpdateRequest request =
                TestDataFactory.createTransactionUpdateRequest(
                        Type.INCOME, new BigDecimal("1500.00"), "Updated", category);
        Transaction existingTransaction = TestDataFactory.createIncomeTransaction();
        Transaction mergedTransaction =
                TestDataFactory.createTransaction(1L, category, new BigDecimal("1500.00"));
        Transaction updatedTransaction =
        ...
        ArgumentCaptor<TransactionUpdateRequest> requestCaptor =
                ArgumentCaptor.forClass(TransactionUpdateRequest.class);
        verify(transactionUpdateRequestMapper, times(1))
    ...
    void suggestCategory_ShouldReturnNoMatchResponse_WhenNoCategoryMatched() {
        ...
        assertThat(result.getBody().message())
                .isEqualTo("No matching category found for this transaction");

        verify(suggestTransactionCategoryUseCase, times(1))
                .execute(request.description(), request.type());
    ...
    void suggestCategory_ShouldHandleIncomeType_WhenIncomeTransactionProvided() {
        ...
        CategorySuggestion suggestion =
                TestDataFactory.createCategorySuggestion(incomeCategory, "high", "Salary");
        CategorySuggestionResponse response =
                TestDataFactory.createCategorySuggestionResponse(

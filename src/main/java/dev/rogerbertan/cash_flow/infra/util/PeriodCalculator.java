...
public class PeriodCalculator {
    ...
    public static DateRange getPreviousPeriod(String period) {
        ...
        return switch (period.toLowerCase()) {
            case "weekly" -> {
                LocalDate previousWeekStart = now.minusWeeks(1)
            ...
            case "quarterly" -> {
                Month currentMonth = now.getMonth();
                Month startMonth = Month.of(((currentMonth.getValue() - 1) / 3) * 3 + 1);
                LocalDate startOfPreviousQuarter = LocalDate.of(now.getYear(), startMonth, 1)
                        .minusMonths(3);
                LocalDate endOfPreviousQuarter = startOfPreviousQuarter.plusMonths(2)
                        .with(TemporalAdjusters.lastDayOfMonth());
            ...
            default -> {
                ...
                LocalDate startOfPreviousMonth = previousMonth.with(TemporalAdjusters.firstDayOfMonth());
                LocalDate endOfPreviousMonth = previousMonth.with(TemporalAdjusters.lastDayOfMonth());

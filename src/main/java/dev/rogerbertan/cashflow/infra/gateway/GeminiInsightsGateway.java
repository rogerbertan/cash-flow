...
import dev.rogerbertan.cashflow.infra.exception.AIInsightsException;
...
public class GeminiInsightsGateway implements AIInsightsGateway {
        ...
        try {
            if (aiProperties.getApiKey() == null || aiProperties.getApiKey().isEmpty()) {
                throw new AIInsightsException(
                        "Gemini API key is not configured. Set GOOGLE_API_KEY environment variable.");
            }
    ...
    public SpendingInsights generateInsights(TransactionAnalysisData analysisData) {
        ...
        try {
            ...
            throw new AIInsightsException(
                    "Failed to generate spending insights: " + e.getMessage(), e);
        }
    }
    ...
    private String buildPrompt(TransactionAnalysisData data) {
        ...
        prompt.append(
                "Based on the transaction data below, provide 5-8 actionable insights about spending behavior.\n\n");

        prompt.append("ANALYSIS PERIOD: ")
                .append(data.period())
                .append(" (")
        ...
        if (data.previousPeriodIncome().compareTo(BigDecimal.ZERO) > 0
            ...
            BigDecimal incomeChange =
                    calculatePercentageChange(data.previousPeriodIncome(), data.totalIncome());
            BigDecimal expenseChange =
                    calculatePercentageChange(data.previousPeriodExpense(), data.totalExpense());

            prompt.append("PREVIOUS PERIOD COMPARISON:\n");
            prompt.append("- Previous Income: $")
                    .append(formatAmount(data.previousPeriodIncome()))
                    .append(" (")
                    .append(formatPercentage(incomeChange))
                    .append(")\n");
            prompt.append("- Previous Expense: $")
                    .append(formatAmount(data.previousPeriodExpense()))
                    .append(" (")
                    .append(formatPercentage(expenseChange))
                    .append(")\n\n");
        }
        ...
        if (!data.categorySummaries().isEmpty()) {
            ...
            data.categorySummaries().stream()
                    ...
                    .forEach(
                            cs ->
                                    prompt.append("- ")
                                            .append(cs.categoryName())
                                            .append(": $")
                                            .append(formatAmount(cs.totalExpense()))
                                            .append("\n"));
            prompt.append("\n");
        }
                    ...
                    .forEach(
                            entry ->
                                    prompt.append("- ")
                                            .append(entry.getKey())
                                            .append(": $")
                                            .append(formatAmount(entry.getValue()))
    ...
    private String callGeminiAPI(String prompt) {
        try {
            GenerateContentResponse response =
                    geminiClient.models.generateContent(aiProperties.getModelName(), prompt, null);
